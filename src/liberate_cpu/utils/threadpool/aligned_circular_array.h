...
// A wrapper.
    template<typename T>
    constexpr size_t alignment_fit(size_t alignment_requirement) {
...
// One more implementation specific specialization: the array size must be a power of 2.
    ...
    class aligned_circular_array {
        ...
        void initialize() {
            aca_info(
                    "Initializer of an aligned circular array has been entered. We will initialize all elements with the default constructor.");
        ...
        typename void_if<is_integral<V>::value>::type destroy() {
            aca_info("Destroyer for intrinsic types has entered. We'll do nothing.");
        };

        template<typename V = T>
        typename void_if<!is_integral<V>::value && !is_pointer<V>::value>::type destroy() {
            aca_info(
                    "Destroyer for derived types(class, struct, and etc) has entered. We'll explicitly call destructors for all elements.");
            for (auto i = 0; i < size; ++i) {
    ...
    public:
        ...
        aligned_circular_array(const aligned_circular_array<T, Align> &other) : aligned_circular_array(other.size) {
            // Copy.
            memcpy(reinterpret_cast<void *>(adjusted_data_ptr), reinterpret_cast<void *>(other.adjusted_data_ptr),
                   utilized_bytes);
        };


        // Move constructor.
        aligned_circular_array(aligned_circular_array<T, Align> &&other) : size{other.size},
        ...
        aligned_circular_array &operator=(const aligned_circular_array<T, Align> &other) {
            ...
            memcpy(reinterpret_cast<void *>(adjusted_data_ptr_new), reinterpret_cast<void *>(other.adjusted_data_ptr),
                   other.utilized_bytes);
            destroy();
            delete[] raw_data_ptr;
        ...
        aligned_circular_array &operator=(nullptr_t) {
            aca_info(
                    "Nullifyer for an aligned circular array has been entered. The user perhaps does not want to destroy the contents pointed at by the array elements. We will simply erase the memory occupied by the array, not calling destructor for each element.");
            delete[] raw_data_ptr;
            size = 0;
            utilized_bytes = 0;
            allocated_bytes = 0;
        ...
        virtual ~aligned_circular_array() {
            aca_info(
                    "Destructor of an aligned circular array has been entered. We will clean up the array buffer if it has one.")
            if (raw_data_ptr) {
                aca_info(
                        "We detected that this aligned circular array indeed has data allocated. We'll destroy elements first.");
                destroy();
