...
class NttContext:
    ...
    def psi_enter(self):
        ...
        ntt_cpu.mont_enter(a, Rs, ql, qh, kl, kh, self.tp, chunk)
    ...
    def mont_enter(
        ...
        L = a[0].size(1)
        chunk = C * L // (self.num_threads * self.thread_load)
        if mont_pack is None:
            mont_pack = self.mont_prepack[mult_type][lvl][part]

        if Rs_pack is None:
            Rs_pack = self.Rs_prepack[mult_type][lvl][part]

        ntt_cpu.mont_enter(a, Rs_pack, *mont_pack, self.tp, chunk)
    ...
    def ntt(self, a, lvl=0, mult_type=-1, part=0):
        ...
        L = a[0].size(1) // 2
        chunk = C * L // (self.num_threads * self.thread_load)
        ntt_cpu.ntt(a, *self.ntt_prepack[mult_type][lvl][part], self.tp, chunk)
    ...
    def reduce_2q_with_parameter(self, a, _2q):
        ...
        L = a[0].size(1)
        chunk = C * L // (self.num_threads * self.thread_load)
        ntt_cpu.reduce_2q(a, _2q, self.tp, chunk)
    ...
    def mont_add(self, a, b, lvl=0, mult_type=-1, part=0, _2q=None):
        ...
        L = a[0].size(1)
        if _2q is None:
            _2q = self._2q_prepack[mult_type][lvl][part]
        chunk = C * L // (self.num_threads * self.thread_load)

        return [ntt_cpu.mont_add(a, b, _2q, self.tp, chunk)]

    def mont_add_2q(self, a, b, _2q, lvl=0, mult_type=-1, part=0):
        C = a[0].size(0)
        L = a[0].size(1)
        chunk = C * L // (self.num_threads * self.thread_load)
        return [ntt_cpu.mont_add(a, b, _2q, self.tp, chunk)]
